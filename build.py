...
def fetch_steam_data(appid):
    ...
    try:
        ...
        return {'name': name, 'description': desc_html, 'short_desc': short_desc,
...
def build_site(reviews):
    ...
    for idx, r in enumerate(reviews):
        ...
        if appid in saved:
            ...
            if not steam:
                ...
                steam = {'appid': appid, 'name': 'Unknown Game (Fetch Failed)',
    ...
    for appid, d in saved.items():
        game_info = {'appid': appid, 'name': d.get('name'), 'header_image': d.get('header_image'),
    ...
    site_games.sort(key=lambda g: (-(g.get('first_seen') or 0), g.get('review_rank', 999999)))

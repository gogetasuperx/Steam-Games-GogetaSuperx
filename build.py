...
def fetch_curator_reviews():
    games = []
    # Fetch the 3 most recent pages (approx 300 reviews)
...
def fetch_steam_data(appid):
    ...
    try:
        ...
        name = soup.find('div', id='appHubTitle').get_text(strip=True) if soup.find('div', id='appHubTitle') else "Unknown Game"
        desc_div = soup.find('div', id='game_area_description')
        desc_html = str(desc_div) if desc_div else "<p>No description available.</p>"
        ...
        return {
            'name': name, 'description': desc_html, 'header_image': f"https://cdn.akamai.steamstatic.com/steam/apps/{appid}/header.jpg",
            'developers': developers, 'publishers': developers, 'release_date': release_date
        }
...
def build_site(games):
    ...
    for g in games:
        ...
        if os.path.exists(json_path):
            with open(json_path, 'r') as f: steam_data = json.load(f)
        else:
        ...
        yt_id = get_yt_id(g['yt_link']) if g['yt_link'] else None
        game_info = {
